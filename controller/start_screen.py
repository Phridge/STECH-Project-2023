...
class StartScreen:
    def __init__(self, events):
        ...
        self.info = ui_elements.BorderedRectangle("Hinweis: Gib mal \"Erster Spielstand\" auf der Tastatur ein!", 10, 17.5, 80, 10, events.color_scheme, color_scheme.Minecraft, 1.9, events, sublist, self.batch)

        self.leave = ui_elements.InputButton("Verlassen", 40, 2.5, 20, 10, events.color_scheme, color_scheme.Minecraft, 7, events, sublist, self.batch)
        ...
        sublist.extend((self.save1.clicked.subscribe(lambda _: self.change_screen("HomeScreen", 1)),
                        self.save2.clicked.subscribe(lambda _: self.change_screen("HomeScreen", 2)),
                        self.save3.clicked.subscribe(lambda _: self.change_screen("HomeScreen", 3)),
                        self.delete_save1.clicked.subscribe(lambda _: self.change_screen("DeleteSaveScreen", 1)),
                        self.delete_save2.clicked.subscribe(lambda _: self.change_screen("DeleteSaveScreen", 2)),
                        self.delete_save3.clicked.subscribe(lambda _: self.change_screen("DeleteSaveScreen", 3)),
                        self.settings.clicked.subscribe(lambda _: self.change_screen("Settings", 0)),
                        self.statistics.clicked.subscribe(lambda _: self.change_screen("Statistics", 0)),
                        self.leave.clicked.subscribe(lambda _: self.change_screen("Restart", False))))
